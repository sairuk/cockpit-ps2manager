...
//                 .-:=[ PS2 Manager ]=:-.
//
// Install/Uninstall games over FTP/HDLDUMP/NDB to PS2
//
// Read the comments throughout this file for more information
//
// - sairuk

var ps2ftpmanager = "/opt/ps2manager/scripts/ps2ftpmanager.sh";
var ps2pfsmanager = "/opt/ps2manager/scripts/ps2pfsmanager.sh";
var ps2hdlmanager = "/opt/ps2manager/scripts/ps2gdlmanager.sh";
var ps2config = "/opt/ps2manager/ps2manager.cfg";
...
function install_options(mode="list-available", xiso=false) {
    ...
    var cmd_os = [
        ps2ftpmanager,
        "-u", rn_settings['ps2_username'],
        "-p", rn_settings['ps2_password'],
        "-i", rn_settings['ps2_ipaddress'],
        "-f", rn_settings['ps2_games_drive'],
        "-t", rn_settings['ps2_games_directory'],
        ...
        items.forEach(item=>{
            ...
            if ( item !== "" ) {
                ...
                if ( mode == "list-available" )
                {
                    ...
                    method_button.addEventListener("click", install_ps2);
                } else if ( mode == "list-installed" ) {
                    mode_button = "uninstall";
                    method_button.addEventListener("click", uninstall_ps2);
                    ...
                    backup_button.addEventListener("click", backup_ps2);
                }
...
// install
function install_ps2() { rn_method(this.id, 'install'); }

// uninstall
function uninstall_ps2() { rn_method(this.id, 'uninstall'); }

// backup
function backup_ps2() { rn_method(this.id, 'backup'); }
...
function update_input_from_cfg() {
    ...
    document.getElementById('s-user-name-input').value = rn_settings['ps2_username'];
    document.getElementById('s-update-passwd-input').value = rn_settings['ps2_password'];

    document.getElementById('s-ip-address-input').value = rn_settings['ps2_ipaddress'];
    document.getElementById('s-set-ps2-install-type-input').value = rn_settings['ps2_type'];

    document.getElementById('s-set-ps2-install-drive-input').value = rn_settings['ps2_games_drive'];
    document.getElementById('s-set-ps2-install-dir-input').value = rn_settings['ps2_games_directory'];
...
function update_cfg_from_input() {
    ...
    rn_settings['ps2_username'] = document.getElementById('s-user-name-input').value;
    rn_settings['ps2_password'] = document.getElementById('s-update-passwd-input').value;

    rn_settings['ps2_ipaddress'] = document.getElementById('s-ip-address-input').value;
    rn_settings['ps2_type'] = document.getElementById('s-set-ps2-install-type-input').value;

    // should be selecting a radio button
    //rn_settings['connect_mode'] = document.getElementById('s-update-connecto-mode-input').value;

    rn_settings['opl_directory'] = document.getElementById('s-update-opl-dir-input').value;
    rn_settings['nbd_device'] = document.getElementById('s-update-nbd-device-input').value;

    rn_settings['ps2_games_drive'] = document.getElementById('s-set-ps2-install-drive-input').value;
    rn_settings['ps2_games_directory'] = document.getElementById('s-set-ps2-install-dir-input').value;
    ...
    rn_settings['local_memcard_directory'] = document.getElementById('s-set-local-memcard-dir-input').value;
}
